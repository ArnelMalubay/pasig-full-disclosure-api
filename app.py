...
from typing import Optional, List, Dict, Any
from bs4 import BeautifulSoup
from pasig_blob_kv_utils import get_current_year, ensure_and_get, get_time
import os
...
async def get_data(
    ...
    if path not in VALID_PATHS:
        raise HTTPException(status_code=404, detail=f"Path '{path}' not found. Valid paths: {', '.join(VALID_PATHS)}")
    ...
    try:
        html_content = ensure_and_get(path)
...
@app.get("/bids-and-awards/{category}", summary="Get bids and awards documents by category")
async def get_bids_and_awards(
    category: str = Path(..., description = "Category of bids/awards document", example="notice-of-awards"),
    query: Optional[str] = Query(None, description="Search query to filter documents by title", example="Rosario"),
    skip: Optional[int] = Query(0, ge=0, description="Number of results to skip for pagination", example=0),
    ...
    if category not in path_to_title:
        raise HTTPException(status_code=404, detail=f"Category '{category}' not found. Valid categories: {', '.join(path_to_title.keys())}")

    try:
        html_content = ensure_and_get("bids-and-awards")
        soup = BeautifulSoup(html_content, "lxml")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error reading data: {str(e)}")

    path_title = path_to_title[category]
    tag_to_use = 'li' if category == 'other-notices' else 'tr'
    headers = soup.find_all(class_="col-md-12 text-center")

    trs = []
    for header in headers:
        if header.h1 and header.h1.string == path_title:
            trs = header.next_sibling.next_sibling.find_all(tag_to_use)
            break
